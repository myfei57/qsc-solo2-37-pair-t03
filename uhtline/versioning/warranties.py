...
class WarrantyBook:
    ...
    def baseline_record(self, baseline_id: str) -> Baseline:
        """Read one baseline record without walking the validity rules."""

        record = self._baselines.get(str(baseline_id))
        if record is None:
            raise NotFoundError("baseline was never issued", kind="baseline", reference=str(baseline_id))
        return Baseline.from_dict(record)
